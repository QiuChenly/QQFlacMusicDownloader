# 20221011, 告知各位本项目已正式停止开发。
# Ooops！this project was stoped!
## 请先阅读以下内容：
由于不可抗力因素 从QMD服务器获取共享绿钻QQ的Cookie的方式已经失败，服务器已关闭。目前本项目只能用于搜索qq音乐中的歌曲列表，并获取相关信息。由于没有绿钻QQ Cookie ，将无法正常使用批量下载。如果您有自己的绿钻Cookie，请不要随意共享给他人，以免造成qq账号被封停。本项目目前依然可用部分功能：
1. 搜索
2. 有绿钻Cookie后依然可以正常批量下载

目前不可用的原因：
1. 无有效的共享绿钻Cookie可用
2. 白嫖党太多了(包括我
3. 其实一年一两百的qq会员并不贵，但是谁让我月薪不足三千呢？只恨自己没有考上好大学 没有努力学习 赚不到钱

# 项目介绍
## 这是一个通过QMD 1.7.2.apk逆向出来的一个批量下载QQ音乐歌曲的脚本, 并没有任何有价值的技术含量。
---
# 使用方法
